...
KEY="duplicate-demo-key"
CORRELATION_ID="failure-smoke-$(date +%s)"
...
curl -fsS -X POST "$API_BASE_URL/dev/failure-injections/reconciliation-mismatch" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: mismatch" \
  -H "Correlation-Id: $CORRELATION_ID" \
  -d '{"enabled":true}' >/dev/null

mismatch="$(curl -fsS -X POST "$API_BASE_URL/sweep-orders" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: mismatch-order" \
  -H "Correlation-Id: $CORRELATION_ID" \
...
curl -fsS -X POST "$API_BASE_URL/dev/failure-injections/reconciliation-mismatch" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: mismatch-off" \
  -H "Correlation-Id: $CORRELATION_ID" \
