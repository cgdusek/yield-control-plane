...
Before load and again before FIS, `make aws-cert-run` executes `scripts/aws-cert-check-fis-targets.sh`. That check lists running worker tasks, describes them with `--include TAGS`, and fails if task tags do not include `CertificationWorkstream=aws-simulation` and `WorkerKind`. This prevents a late `Target resolution returned empty set` FIS failure after the 30-minute k6 campaign. If this gate fails after changing ECS tag propagation, redeploy with the default forced ECS deployment path so ECS replaces any already-running untagged tasks.
